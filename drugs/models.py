...
#create tab drugs class ineheriting Drugs
...
class Sale(models.Model):
	...
	def compute(self, tab):
		self.total_price = self.drug.price * self.amount
		if tab:
			self.total_price = self.total_price / int(self.drug.get_tab_cd()[0])
			print(int(self.drug.get_tab_cd()[0]))
			self.drug_name += " - TAB"
